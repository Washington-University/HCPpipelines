...
for Subject in $Subjlist ; do
  ...
  NumberOfSamples="100" #1 sample then 2 and calculate total time required per sample
  ...
  for Hemisphere in $HemisphereSTRING ; do
    fsl_sub -q long.q "$GitRepo"/DiffusionTractography/scripts/RunMatrix3Tractography.sh "$StudyFolder" "$Subject" "$DownSampleNameI" "$DiffusionResolution" "$Caret7_Command" "$Hemisphere" "$NumberOfSamples" "$StepSize" "$Curvature" "$DistanceThreshold" "$GlobalBinariesDir"
