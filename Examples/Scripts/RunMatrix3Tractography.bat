...
for Subject in $Subjlist ; do
  ...
  for Hemisphere in $HemisphereSTRING ; do
    #fsl_sub -q long.q "$GitRepo"/RunMatrix3Tractography.sh "$StudyFolder" "$Subject" "$DownSampleNameI" "$DiffusionResolution" "$Caret7_Command" "$Hemisphere" "$NumberOfSamples" "$StepSize" "$Curvature" "$DistanceThreshold" "$GlobalBinariesDir"
