...
for Subject in $Subjlist ; do
  ...
  /home/NRG/jwilso01/dev/Pipelines/FreeSurfer/FreeSurferPipeline.sh "$SubjectID" "$SubjectDIR" "$T1wImage" "$T1wImageBrain" "$T2wImage" "$PipelineScripts" "$PipelineBinaries" "$Caret5_Command" "$Caret7_Command" 
done
