...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if b_verbose, fprintf(['Starting ICA calculation for ' num2str(numOfIC) ' Components...\n']); end
...
if approachMode == 1,
    ...
    for trynum = 1:maxtries
        ...
        if round == maxNumIterations + 1,
            fprintf('No convergence after %d steps, restarting\n', maxNumIterations);
    ...
    if trynum == maxtries
        fprintf('No convergence after %d retries, dying\n', maxtries);
        %if ~isempty(B)
            % Symmetric orthogonalization.
            %B = B * real(inv(B' * B)^(1/2));
