...
get_options()
{
	...
	DiffTopupConfig="${DiffTopupConfig:-$HCPPIPEDIR/global/config/b02b0.cnf}"
	SpeciesLabel="${SpeciesLabel:-0}"
	...
	echo "   runcmd: ${runcmd}"
	echo "   TopupConfig: ${DiffTopupConfig}"
...
main()
{
	...
	for Image in ${PosInputImages} ; do
		...
		if [ ${Image} = ${MissingFileFlag} ] ; then
			...
			# correct/reorient input data - TH Aug 2024
	...
	if [[ $(imtest ${StudyFolder}/${Subject}/T2w/T2w.nii.gz) = 1 && "${UsePhaseZero}" = "true" ]] ; then
                T2wImage=${StudyFolder}/${Subject}/T2w/T2w.nii.gz
                ...
                immv ${outdir}/topup/Pos_Neg_b0  ${outdir}/topup/Pos_Neg_NoZero_b0
	...
	${runcmd} ${HCPPIPEDIR_dMRI}/run_topupNHP.sh ${outdir}/topup ${DiffTopupConfig} ${SpeciesLabel}
