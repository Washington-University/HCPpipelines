...
PARAMETERs are: [ ] = optional; < > = user supplied value
  ...
  [--combine-data-flag=<value>]
                          ...
                          Defaults to 1
  [--topupconfig=<topup config>]
                          Use the specified topup config (default b0b0.conf)

  [--specieslabel=<label>]
                          Species label (0: Human, 1: Chimp, 2: Macaque, 3: Marmoset
                          4: Night monkey, 5: Rat, 6: Mouse, 7: Mini pig)
                          Default to 0
  [--wmprojabs=<value>]
                          white matter projection absolute depth
...
get_options()
{
		...
		case ${argument} in
			...
			--resamp=*)
				...
				index=$((index + 1))
			        ;;
			--truepatientposition=*)
				TruePatientPosition=${argument#*=}
				index=$(( index + 1 ))
				;;
			--scannerpatientposition=*)
				ScannerPatientPosition=${argument#*=}
				index=$(( index + 1 ))
				;;
			--topupconfig=*)
			...
			--usephasezero)
				...
				;;
                        --specieslabel=*)
                                SpeciesLabel=${argument#*=}
                                index=$(( index + 1 ))
                                ;;
                        --wmprojabs=*)
                                DiffWMProjAbs=${argument#*=}
                                index=$(( index + 1 ))
                                ;;
			*)
				usage
	...
	echo "   TruePatientPosition: ${TruePatientPosition}"
	echo "   ScannerPatientPosition: ${ScannerPatientPosition}"
	echo "   ToupupConfig: ${TopupConfig}"
	echo "   SpeciesLabel: ${SpeciesLabel}"
	echo "   DiffWMProjAbs: ${DiffWMProjAbs}"
...
main()
{
	...
	pre_eddy_cmd+=" --truepatientposition=${TruePatientPosition} "
	pre_eddy_cmd+=" --scannerpatientposition=${ScannerPatientPosition} "
	pre_eddy_cmd+=" --topupconfig=${TopupConfig}"
	pre_eddy_cmd+=" --specieslabel=${SpeciesLabel}"
	...
	if [ ! -z "${extra_eddy_args}" ] ; then
        ...
        if [ ! -z "${resamp_value}" ] ; then # TH adeded for RIKEN, Feb 2023
	...
	post_eddy_cmd+=" --specieslabel=${SpeciesLabel} "
	post_eddy_cmd+=" --wmprojabs=${DiffWMProjAbs} "
