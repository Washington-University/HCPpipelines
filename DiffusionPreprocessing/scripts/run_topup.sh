...
if [ "${SpeciesLabel}" = "0" ]; then
	...
	# NHP pathway: parallel topup + optional T2w-phase-zero variant + per-species BET fraction - TH Jan 2023
	echo -e "\n About to run topup"
	...
	echo " Running topup"
	${FSLDIR}/bin/topup --nthr=6 --imain=${workingdir}/Pos_Neg_b0 --datain=${workingdir}/acqparams.txt --config=${topup_config_file} --out=${workingdir}/topup_Pos_Neg_b0 -v --fout=${workingdir}/topup_Pos_Neg_b0_field.nii.gz --iout=${workingdir}/topup_Pos_Neg_b0
	...
	if [ $($FSLDIR/bin/imtest ${workingdir}/Pos_Neg_NoZero_b0.nii.gz) = 1 ] ; then # No T2w registration
		echo " Running topup with no phase zero"
		${FSLDIR}/bin/topup --nthr=6 --imain=${workingdir}/Pos_Neg_NoZero_b0 --datain=${workingdir}/acqparams_NoZero.txt --config=${topup_config_file} --out=${workingdir}/topup_Pos_Neg_NoZero_b0 -v --fout=${workingdir}/topup_Pos_Neg_NoZero_b0_field.nii.gz --iout=${workingdir}/topup_Pos_Neg_NoZero_b0
