...
# Output Variables
T1wOutputDirectory=`getopt1 "--datadiffT1wdir" $@`# "$11" #Path to T1w space diffusion data (for producing output)
RegOutput=`getopt1 "--regoutput" $@`              # "$12" #Temporary file for sanity checks 
QAImage=`getopt1 "--QAimage" $@`                  # "$13" #Temporary file for sanity checks 
...
#Register diffusion data to T1w space without considering gradient nonlinearities
    ${FSLDIR}/bin/flirt -in "$DataDirectory"/data -ref "$T1wRestoreImage"_1.25 -applyxfm -init "$WorkingDirectory"/diff2str.mat -interp spline -out "$T1wOutputDirectory"/data
fi
