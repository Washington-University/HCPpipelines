...
set -e
if [ "$HCPPIPEDIR" = "" ] ; then
	echo "Please export HCPPIPEDIR before running this script"
	exit 1;
fi
...
main()
{
	...
	${runcmd} ${HCPPIPEDIR_dMRI}/eddy_postprocNHP.sh ${outdir} ${GdCoeffs} ${CombineDataFlag} ${SelectBestB0} ${SpeciesLabel}
	...
	log_Msg "Command: ${HCPPIPEDIR_dMRI}/DiffusionToStructuralNHP.sh \
		--t1folder=${T1wFolder} \
		--subject=${Subject} \
		--workingdir=${outdir}/reg \
		--datadiffdir=${outdir}/data \
	...
	if [ "$(ls ${outdir}/QC/)" != "" ] ; then 
       cp -p ${outdir}/QC/* ${outdirT1w}/QC
	fi
	imcp ${outdir}/data/cnr_maps ${outdirT1w}/QC/cnr_maps
