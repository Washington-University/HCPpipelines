...
class XGBoostEnsembleClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        y_1_indices = np.where(y == 1)[0]
        for model in self.models:
            y_0_random_indices = np.random.choice(y_0_indices, size=int(0.7*len(y_1_indices)), replace=False)
            y_1_random_indices = np.random.choice(y_1_indices, size=int(0.7*len(y_1_indices)), replace=False)
