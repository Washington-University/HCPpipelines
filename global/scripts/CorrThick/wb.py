...
PHYSICALDIR = os.environ.get("PHYSICALDIR", "T1w")

def wb_metric_resample_to_164k(subjects_dir,subject,hemi,surface,mesh):

    input_file = os.path.join(subjects_dir, subject,STANDARDDIR,'Native','{sub}.{h}.thickness.native.shape.gii'.format(sub=subject,h=hemi))
    input_sphere = os.path.join(subjects_dir, subject,STANDARDDIR,'Native','{sub}.{h}.sphere.native.surf.gii'.format(sub=subject,h=hemi))
    output_sphere = os.path.join(subjects_dir, subject,STANDARDDIR,'{sub}.{h}.sphere.164k_fs_LR.surf.gii'.format(sub=subject,h=hemi))
    output_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.thickness.{m}.resample.shape.gii'.format(sub=subject,h=hemi,m=mesh))
    input_surf = os.path.join(subjects_dir, subject,PHYSICALDIR,'Native','{sub}.{h}.{s}.native.surf.gii'.format(sub=subject,s=surface,h=hemi))
    output_surf = os.path.join(subjects_dir, subject,STANDARDDIR,'{sub}.{h}.{s}.164k_fs_LR.surf.gii'.format(sub=subject,s=surface,h=hemi))
    roi_file = os.path.join(subjects_dir, subject,STANDARDDIR,'Native','{sub}.{h}.roi.native.shape.gii'.format(sub=subject,h=hemi))
...
def wb_surf_resample_to_164k(subjects_dir,subject,hemi,surface,mesh):
    ...
    input_file = os.path.join(subjects_dir, subject,PHYSICALDIR,'Native','{sub}.{h}.{s}.native.surf.gii'.format(sub=subject,s=surface,h=hemi))
    input_sphere = os.path.join(subjects_dir, subject,STANDARDDIR,'Native','{sub}.{h}.sphere.native.surf.gii'.format(sub=subject,h=hemi))
    output_sphere = os.path.join(subjects_dir, subject,STANDARDDIR,'{sub}.{h}.sphere.164k_fs_LR.surf.gii'.format(sub=subject,h=hemi))
    output_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.{s}.{m}.resample.surf.gii'.format(sub=subject,h=hemi,s=surface,m=mesh))
...
def wb_taubin(subjects_dir,subject,hemi,surface,mesh,iteration):
    ...
    output_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.{s}.{m}.func.gii'.format(sub=subject,h=hemi,s=surface,m=mesh))
    surf_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.{s}.{m}.resample.surf.gii'.format(sub=subject,h=hemi,s=surface,m=mesh))
    command = "wb_command -surface-coordinates-to-metric {s} {o}".format(s=surf_file, o=output_file)
    os.system(command) 
    
    output_file1 = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.{s}.it.{m}.func.gii'.format(sub=subject,h=hemi,s=surface,m=mesh))
    command = "wb_command -metric-smoothing {s} {o} {it} {o1} -fwhm".format(s=surf_file,o=output_file,it=iteration, o1=output_file1)
    os.system(command) 
    
    output_file2 = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.{s}.it.it.{m}.func.gii'.format(sub=subject,h=hemi,s=surface,m=mesh))
    command = "wb_command -metric-smoothing {s} {o1} {it} {o2} -fwhm".format(s=surf_file,o1=output_file1,it=iteration,o2=output_file2)
    os.system(command) 
    
    output_file3 = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.{s}.smooth.{m}.func.gii'.format(sub=subject,h=hemi,s=surface,m=mesh))
    ...
    output_file4 = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.{s}.{m}.resample.smooth.surf.gii'.format(sub=subject,h=hemi,s=surface,m=mesh))
...
def wb_smooth(subjects_dir,subject,hemi,surface,mesh,smooth):
    ...
    for curv in curvs:
        output_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.{s}.{c}.smooth.shape.gii'.format(sub=subject,h=hemi,s=surface,c=curv))
        surf_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.{s}.{m}.resample.smooth.surf.gii'.format(sub=subject,h=hemi,s=surface,m=mesh))
        input_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', '{sub}.{h}.{s}.{c}.shape.gii'.format(sub=subject,h=hemi,s=surface,c=curv))
...
def wb_rois(subjects_dir,subject,hemi,surface,mesh,number):
    ...
    surf_data = nib.load(os.path.join(subjects_dir, subject,STANDARDDIR,'Native', 'CorrThick','{sub}.{h}.{s}.{m}.resample.smooth.surf.gii'.format(sub=subject,h=hemi,s=surface,m=mesh))).agg_data()
    indices = np.arange(len(surf_data[0]))
    
    vert_list_name = '{sub}.{h}.{s}.vert.{m}.asc'.format(sub=subject,h=hemi,s=surface,m=mesh)
    vert_list = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick', vert_list_name)
    np.savetxt(vert_list, indices, fmt='%00d', delimiter=' '' ') 
    
    #take "number" as HWHM, convert via FWHM
    sigma = number / math.sqrt(2 * math.log(2))
    limit = sigma * 3
    output_file = os.path.join(subjects_dir, subject,STANDARDDIR,'Native','CorrThick','{sub}.{h}.{s}.roi.{m}.csv'.format(sub=subject,h=hemi,s=surface,n=number,m=mesh))
    surf_file = os.path.join(subjects_dir, subject,STANDARDDIR,'Native','CorrThick','{sub}.{h}.{s}.{m}.resample.smooth.surf.gii'.format(sub=subject,h=hemi,s=surface,m=mesh))
    vert_file = os.path.join(subjects_dir,subject, STANDARDDIR,'Native','CorrThick','{sub}.{h}.{s}.vert.{m}.asc'.format(sub=subject,h=hemi,s=surface,m=mesh))
...
def wb_metric_resample_to_native(subjects_dir,subject,hemi,surface):
    ...
    for i in range(len(input_names)):
        ...
        input_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick','{sub}.{h}.{s}.{inp}.shape.gii'.format(sub=subject,h=hemi,s=surface,inp=input_names[i]))
        input_sphere = os.path.join(subjects_dir, subject,STANDARDDIR,'{sub}.{h}.sphere.164k_fs_LR.surf.gii'.format(sub=subject,h=hemi))
        output_sphere = os.path.join(subjects_dir, subject,STANDARDDIR,'Native','{sub}.{h}.sphere.native.surf.gii'.format(sub=subject,h=hemi))
        output_file = os.path.join(subjects_dir, subject,STANDARDDIR,'Native','{sub}.{h}.{out}.native.shape.gii'.format(sub=subject,h=hemi,s=surface,out=output_names[i]))
        input_surf = os.path.join(subjects_dir, subject,STANDARDDIR,'{sub}.{h}.{s}.164k_fs_LR.surf.gii'.format(sub=subject,s=surface,h=hemi))
        output_surf = os.path.join(subjects_dir, subject,PHYSICALDIR,'Native','{sub}.{h}.{s}.native.surf.gii'.format(sub=subject,s=surface,h=hemi))
...
def wb_set_map_names(subjects_dir,subject,hemi): 
    ...
    input_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', input_file)
...
def wb_structure(subjects_dir,subject,hemi,surface,structure):
    ...
    for curv in curvs:
        input_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native', 'CorrThick','{sub}.{h}.{s}.{c}.smooth.shape.gii'.format(sub=subject,h=hemi,s=surface,c=curv))
    ...
    for name in names:
        input_file = os.path.join(subjects_dir, subject, STANDARDDIR,'Native','{sub}.{h}.{inp}.native.shape.gii'.format(sub=subject,h=hemi,s=surface,inp=name))
        command = "wb_command -set-structure {i} {s}".format(s=structure, i=input_file)
        os.system(command)
