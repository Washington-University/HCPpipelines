function [Iq,A,W,S,sR]=icasso(mode,X,M,varargin)
%function [Iq,A,W,S,sR]=icasso(mode,X,M,['argid1',value1,'argid2',value2,...])
%
%PURPOSE
%
   ...
   case 'vis'
    ...
    switch lower(vis),
     case {'off','basic','none'}
      ; % ok
     otherwise
      error('Option ''vis'' must be ''basic,'' ''off,'' or ''none''.');
    end
   otherwise
    fasticaoptions(j:j+1)=varargin(i:i+1);
%    fasticaoptions=varargin;
    j=j+2;
  end
end
...
if isempty(fasticaoptions)
  sR=icassoEst(mode,X,M)
else
  sR=icassoEst(mode,X,M,fasticaoptions{:});
end

%%% 2. Compute & visualize/return results

L=icassoGet(sR,'rdim');
L=mean(icassoGet(sR,'numOfIC'));
