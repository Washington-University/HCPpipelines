...
Usage() {
  echo "`basename $0`: Detect veins in 7T T1w images (optionally with the help of T2w images)"
  echo " "
  echo "Usage: `basename $0` [--workingdir=<working dir>] --t1=<input T1w image> --t1brain=<input T1w brain image> [--brainmaskloose=<loose brain mask for the veins>] [--exclmask=<exclusion mask (e.g. WM) image>] [--basename=<output base name>]"
}
...
# default parameters
WD=$(defaultopt $WD ./VeinDetection)
...
echo "  finding veins based on their intensity in the T1w image"
...
if [[ -z $ExclMask ]] ; then
  echo "  Not excluding any voxels from the vein mask (such as WM voxels). This is potentially problematic, so stricter vein detection criteria are applied."
  cluster90thr=200
  cluster99thr=100
else
...
${FSLDIR}/bin/fslmaths $WD/${BaseOut}_thr90csm -thr $cluster90thr -bin $WD/${BaseOut}_vein90
${FSLDIR}/bin/fslmaths $WD/${BaseOut}_thr99csm -thr $cluster99thr -bin $WD/${BaseOut}_vein99

# add voxels with extreme values, regardless of their cluster extent

echo "  dilating vein skeletons"
# repeatedly dilate the high-threshold clusters one voxel into the low-intensity clusters
${FSLDIR}/bin/fslmaths $WD/${BaseOut}_vein99 -dilF -mas $WD/${BaseOut}_vein90 $WD/${BaseOut}_veinmask
${FSLDIR}/bin/fslmaths $WD/${BaseOut}_veinmask -dilF -mas $WD/${BaseOut}_vein90 $WD/${BaseOut}_veinmask
${FSLDIR}/bin/fslmaths $WD/${BaseOut}_veinmask -dilF -mas $WD/${BaseOut}_vein90 $WD/${BaseOut}_veinmask

# dilate the whole mask, regardless of low-intensity clusters
${FSLDIR}/bin/fslmaths $WD/${BaseOut}_veinmask -dilF $WD/${BaseOut}_veinmaskdil

# invert the masks to define voxels that are not veins
${FSLDIR}/bin/fslmaths $WD/${BaseOut}_veinmask -sub 1 -mul -1 $WD/${BaseOut}_veinmask_inv
${FSLDIR}/bin/fslmaths $WD/${BaseOut}_veinmaskdil -sub 1 -mul -1 $WD/${BaseOut}_veinmaskdil_inv

# cleanup intermediate images
${FSLDIR}/bin/imrm $T1wBrainLoose $WD/${BaseOut}_thr90cs $WD/${BaseOut}_thr90cm $WD/${BaseOut}_thr90csm $WD/${BaseOut}_thr99cs $WD/${BaseOut}_thr99cm $WD/${BaseOut}_thr99csm $WD/${BaseOut}_vein90 $WD/${BaseOut}_vein99

# reporting
echo " "
echo " END: Vein Detection"
...
echo "fslview $T1w $WD/${BaseOut}_veinmask" >> $WD/qa.txt
