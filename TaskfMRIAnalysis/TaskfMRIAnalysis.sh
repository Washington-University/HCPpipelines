...
opts_AddMandatory '--lvl1tasks' 'LevelOnefMRINames' 'tfMRI_TaskName_RL@tfMRI_TaskName_LR' "List of subdirectories containing each time series image for the TaskName task. Multiple scan subdirectories should be provided as a single string separated by '@' character." #Assumes these subdirectories are located in the SubjectID/MNINonLinear/Results directory. Also assumes that timeseries image filename begins with this string.
opts_AddOptional '--lvl1fsfs' 'LevelOnefsfNames' 'DesignName_TaskName_RL@DesignName_TaskName_LR' "List of prefixes in the design.fsf filenames for each time series image for the TaskName task. Should contain same number of design files as time series images in --lvl1tasks option. (N-th design will be used for N-th time series image.) Separate multiple design files by '@' character. If no value is passed to --lvl1fsfs, the value will be set to the same list passed to --lvl1tasks."
...
opts_AddOptional '--lowresmesh' 'LowResMesh' 'integer' "Default=32, which is appropriate for HCP minimal preprocessing pipeline outputs" '32'
opts_AddOptional '--grayordinatesres' 'GrayordinatesResolution' 'integer' "Default=2mm, which is appropriate for HCP minimal preprocessing pipeline outputs" '2'
opts_AddOptional '--origsmoothingFWHM' 'OriginalSmoothingFWHM' 'integer' "Default=2mm, which is appropriate for HCP minimal preprocessing pipeline outputs" '2'
opts_AddOptional '--finalsmoothingFWHM' 'FinalSmoothingFWHM' 'integer' "Default=2mm, which is no additional smoothing above HCP minimal preprocessing pipelines outputs. Total desired smoothing (finalsmoothingFWHM) is reached by applying additional smoothing onto origsmoothingFWHM." '2'
opts_AddOptional '--confound' 'Confound' 'filename' "Confound matrix text filename (e.g., output of fsl_motion_outliers). Assumes file is located in SubjectID/MNINonLinear/Results/tfMRI_TaskName_Scan. Default='NONE'" 'NONE'
opts_AddOptional '--highpassfilter' 'TemporalFilter' 'integer' "Value for nonlinear highpass filter (in seconds). Default=200. Use 2000 for linear detrend." '200'
...
opts_AddOptional '--regname' 'RegName' 'RegName' "Name of surface registration technique. Default=NONE, which will use the default surface registration." 'NONE'
