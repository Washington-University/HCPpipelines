...
mcflirt -in "$InputfMRI" -reffile "$Scout" -out "$WorkingDirectory"/"$OutputfMRIFile" -mats -plots -rmsrel -rmsabs
mv "$WorkingDirectory"/"$OutputfMRIFile".mat $OutputMotionMatrixFolder
...
if [ -e $OutputMotionMatrixFolder ] ; then
  ...
  for Matrix in $Matrices ; do
    ...
    mv $Matrix "$OutputMotionMatrixNamePrefix""$MatrixNumber"
  done
  cd $DIR
fi
