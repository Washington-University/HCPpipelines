...
# Takuya Hayashi, RIKEN BCIL 2016-2023
...
mkdir -p $tmpdir
echo $(date -R) >> $tmpdir/log.txt
echo "$command" >> $tmpdir/log.txt
echo "============================" >> $tmpdir/log.txt
echo "" >> $tmpdir/log.txt

# convert from mgz to nifti
echo "Convert from .mgz to .nii.gz..."
...
if [ $method = FAST ] ; then
	...
	stronbiasflag="--strongbias"

	echo "Run fsl_anat..."
	${FSLDIR}/bin/fsl_anat -i "$tmpdir"/orig_brain -o "$tmpdir"/orig_brain --nobet --noreorient --clobber --nocrop --noreg --nononlinreg --noseg --nosubcortseg -s ${lowpass} --nocleanup -t $type $stronbiasflag
