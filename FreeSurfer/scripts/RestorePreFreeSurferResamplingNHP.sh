...
if [ -e "$T2wFolder"/T2wToT1wDistortionCorrectAndReg ] ; then 
	log_Msg "  restoring T1w_acpc_dc & T1w_acpc_dc_brain"
...
log_Msg "  restoring T1w_acpc_dc_restore & T1w_acpc_dc_restore_brain"
...
if [ $(${FSLDIR}/bin/imtest "${T2wFolder}"/T2w_acpc) = 1 ] ; then
	log_Msg "  restoring T2w_acpc_dc_restore & T2w_acpc_dc_restore_brain"
...
log_Msg "END: RestorePreFreeSurferResamplingNHP.sh"
