...
if [ $fMRIReference = "NONE" ] ; then
  ...
  ${RUN} ${PipelineScripts}/DistortionCorrectionAndEPIToT1wReg_FLIRTBBRAndFreeSurferBBRbased.sh \
    ...
    ${FSLDIR}/bin/imcp ${fMRIReferencePath}/${QAImage} ${fMRIFolder}/${QAImage}
    # -- Note: we are copying the SBRef as it is the one used in this case and it won't be generated by OneStepResampling
    ${FSLDIR}/bin/imcp ${fMRIReferencePath}/${fMRIReference}_SBRef_nonlin ${fMRIFolder}/${NameOffMRI}_SBRef_nonlin
fi
