...
for Mesh in native 512 2k 8k 18k ; do
    ...
    for Structure in $Structures ; do
        ...
        for Hemisphere in L R ; do
            ...
            for SurfaceEntry in $Surfaces ; do
                ...
                wb_command -add-to-spec-file "${PhysicalHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" "${HemiStructure}" "${PhysicalHippUnfoldFolder}/${Subject}.${Hemisphere}.${Structure}_${Surface}.${Mesh}.surf.gii"
                ...
                wb_command -add-to-spec-file "${AtlasHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" "${HemiStructure}" "${AtlasHippUnfoldFolder}/${Subject}.${Hemisphere}.${Structure}_${Surface}.${Mesh}.surf.gii"
            done
            ...
            wb_command -add-to-spec-file "${PhysicalHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" "${HemiStructure}" "${PhysicalHippUnfoldFolder}/${Subject}.${Hemisphere}.${Structure}_flat.${Mesh}.surf.gii"
            ...
            wb_command -add-to-spec-file "${AtlasHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" "${HemiStructure}" "${AtlasHippUnfoldFolder}/${Subject}.${Hemisphere}.${Structure}_flat.${Mesh}.surf.gii"
    ...
    for ScalarEntry in $Scalars ; do
        ...
        wb_command -cifti-create-dense-scalar "${PhysicalHippUnfoldFolder}/${Subject}.hippocampus_${Scalar}.${Mesh}.dscalar.nii" \
            -metric HIPPOCAMPUS_LEFT "${PhysicalHippUnfoldFolder}/${Subject}.L.hipp_${Scalar}.${Mesh}.shape.gii" \
            -metric HIPPOCAMPUS_RIGHT "${PhysicalHippUnfoldFolder}/${Subject}.R.hipp_${Scalar}.${Mesh}.shape.gii" \
            -metric HIPPOCAMPUS_DENTATE_LEFT "${PhysicalHippUnfoldFolder}/${Subject}.L.dentate_${Scalar}.${Mesh}.shape.gii" \
            -metric HIPPOCAMPUS_DENTATE_RIGHT "${PhysicalHippUnfoldFolder}/${Subject}.R.dentate_${Scalar}.${Mesh}.shape.gii"
        ...
        wb_command -add-to-spec-file "${PhysicalHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" INVALID "${PhysicalHippUnfoldFolder}/${Subject}.hippocampus_${Scalar}.${Mesh}.dscalar.nii"

        cp "${PhysicalHippUnfoldFolder}/${Subject}.hippocampus_${Scalar}.${Mesh}.dscalar.nii" "${AtlasHippUnfoldFolder}/${Subject}.hippocampus_${Scalar}.${Mesh}.dscalar.nii"

        wb_command -add-to-spec-file "${AtlasHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" INVALID "${AtlasHippUnfoldFolder}/${Subject}.hippocampus_${Scalar}.${Mesh}.dscalar.nii"
    done
    ...
    for LabelEntry in $Labels ; do
        ...
        wb_command -cifti-create-label "${PhysicalHippUnfoldFolder}/${Subject}.hippocampus_${Label}.${Mesh}.dlabel.nii" \
            -label HIPPOCAMPUS_LEFT "${PhysicalHippUnfoldFolder}/${Subject}.L.hipp_${Label}.${Mesh}.label.gii" \
            -label HIPPOCAMPUS_RIGHT "${PhysicalHippUnfoldFolder}/${Subject}.R.hipp_${Label}.${Mesh}.label.gii" \
            -label HIPPOCAMPUS_DENTATE_LEFT "${PhysicalHippUnfoldFolder}/${Subject}.L.dentate_${Label}.${Mesh}.label.gii" \
            -label HIPPOCAMPUS_DENTATE_RIGHT "${PhysicalHippUnfoldFolder}/${Subject}.R.dentate_${Label}.${Mesh}.label.gii"

        wb_command -set-map-names "${PhysicalHippUnfoldFolder}/${Subject}.hippocampus_${Label}.${Mesh}.dlabel.nii" -map 1 "${Subject}_${Name}"

        wb_command -add-to-spec-file "${PhysicalHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" INVALID "${PhysicalHippUnfoldFolder}/${Subject}.hippocampus_${Label}.${Mesh}.dlabel.nii"

        cp "${PhysicalHippUnfoldFolder}/${Subject}.hippocampus_${Label}.${Mesh}.dlabel.nii" "${AtlasHippUnfoldFolder}/${Subject}.hippocampus_${Label}.${Mesh}.dlabel.nii"

        wb_command -add-to-spec-file "${AtlasHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" INVALID "${AtlasHippUnfoldFolder}/${Subject}.hippocampus_${Label}.${Mesh}.dlabel.nii"
    done
    ...
    wb_command -add-to-spec-file "${PhysicalHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" INVALID "${PhysicalHippUnfoldFolder}/${Subject}.HippocampalSubfields.nii.gz"

    wb_command -volume-resample "${PhysicalHippUnfoldFolder}/${Subject}.HippocampalSubfields.nii.gz" "${AtlasFolder}/T1w_restore.nii.gz" \
        ENCLOSING_VOXEL "${AtlasHippUnfoldFolder}/${Subject}.HippocampalSubfields.nii.gz" -warp "${AtlasFolder}/xfms/acpc_dc2standard.nii.gz" -fnirt "${AtlasFolder}/T1w_restore.nii.gz"

    wb_command -add-to-spec-file "${AtlasHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" INVALID "${AtlasHippUnfoldFolder}/${Subject}.HippocampalSubfields.nii.gz"

    #NIFTI Input Volumes
    wb_command -add-to-spec-file "${PhysicalHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" INVALID "${T1wFolder}/T1w_acpc_dc_restore.nii.gz"

    wb_command -add-to-spec-file "${PhysicalHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" INVALID "${T1wFolder}/T2w_acpc_dc_restore.nii.gz"

    wb_command -add-to-spec-file "${AtlasHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" INVALID "${AtlasFolder}/T1w_restore.nii.gz"

    wb_command -add-to-spec-file "${AtlasHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" INVALID "${AtlasFolder}/T2w_restore.nii.gz"

    # Merge hippocampal spec files with the corresponding cortical MSMAll spec files
    if [[ "${Mesh}" = "native" ]] ; then

        wb_command -spec-file-merge "${PhysicalHippUnfoldFolder}/${Subject}.${Mesh}.wb_spec" "${T1wFolder}/Native/${Subject}.native.wb.spec" "${PhysicalHippUnfoldFolder}/${Subject}.${Mesh}.native.wb_spec"
