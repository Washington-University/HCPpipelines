  ...
  for fMRIName in $fMRINames ; do
    ...
    if [ ${RevertBiasField} = "YES" ] ; then
      if [ ! -e ${ResultsFolder}/${fMRIName}${fMRIProcSTRING}_bias.dscalar.nii ] ; then
        echo "Bias field in CIFTI space with correct file name doesn't exist:"
        echo "${ResultsFolder}/${fMRIName}${fMRIProcSTRING}_bias.dscalar.nii"
        echo "You need to run RestingStateStats to generate it or set RevertBiasField to NO"
        exit
      fi
...
M_PROG
echo "ComputeVN('${cleandtseries}','${bias}','${ICAtcs}','${ICANoise}','${OutputVN}','${Caret7_Command}');"

      else
        if [ ! -e ${ResultsFolder}/${fMRIName}${fMRIProcSTRING}_vn.dscalar.nii ] ; then
          echo "Variance Normalization file doesn't exist:"
          echo "${ResultsFolder}/${fMRIName}${fMRIProcSTRING}_vn.dscalar.nii"
          echo "You need to run RestingStateStats to generate it or set ComputeVarianceNormalization to YES"
          exit
        fi
