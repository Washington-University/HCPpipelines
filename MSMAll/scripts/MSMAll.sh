...
get_options()
{
	...
	while [ ${index} -lt ${num_args} ]; do
		...
		case ${argument} in
			...
			--rsn-cost-weights=*)
				...
				;;
			--myelin-target-file=*)
				g_myelin_target_fiile=${argument/*=/""}
				index=$(( index + 1 ))
				;;
			--topography-roi-file=*)
	...
	if [ -z "${g_path_to_study_folder}" ]; then
		...
		echo "g_path_to_study_folder: ${g_path_to_study_folder}"
	fi
	...
	if [ -z "${g_high_res_mesh}" ]; then
		...
		echo "g_high_res_mesh: ${g_high_res_mesh}"
	fi
	...
	if [ -z "${g_low_res_mesh}" ]; then
		...
		echo "g_low_res_mesh: ${g_low_res_mesh}"
	fi
	...
	if [ -z "${g_fmri_names_list}" ]; then
		...
		echo "g_fmri_names_list: ${g_fmri_names_list}"
	fi
	...
	if [ -z "${g_output_fmri_name}" ]; then
		...
		echo "g_output_fmri_name: ${g_output_fmri_name}"
	fi
	...
	if [ -z "${g_fmri_proc_string}" ]; then
		...
		echo "g_fmri_proc_string: ${g_fmri_proc_string}"
	fi
	...
	if [ -z "${g_input_pca_registration_name}" ]; then
		...
		echo "g_input_pca_registration_name: ${g_input_pca_registration_name}"
	fi
	...
	if [ -z "${g_input_registration_name}" ]; then
		...
		echo "g_input_registration_name: ${g_input_registration_name}"
	fi
	...
	if [ -z "${g_registration_name_stem}" ]; then
		...
		echo "g_registration_name_stem: ${g_registration_name_stem}"
	fi
	...
	if [ -z "${g_rsn_target_file}" ]; then
		...
		echo "g_rsn_target_file: ${g_rsn_target_file}"
	fi
	...
	if [ -z "${g_rsn_cost_weights}" ]; then
		...
		echo "g_rsn_cost_weights: ${g_rsn_cost_weights}"
	fi

	if [ -z "${g_myelin_target_fiile}" ]; then
		echo "ERROR: myelin_target_fiile required"
		error_count=$(( error_count + 1 ))
	else
		echo "g_myelin_target_fiile: ${g_myelin_target_fiile}"
	fi
	...
	if [ -z "${g_topography_roi_file}" ]; then
		...
		echo "g_topography_roi_file: ${g_topography_roi_file}"
	fi
	...
	if [ -z "${g_topography_target_file}" ]; then
		...
		echo "g_topography_target_file: ${g_topography_target_file}"
	fi
	...
	if [ -z "${g_iterations}" ]; then
		...
		echo "g_iterations: ${g_iterations}"
	fi
	...
	if [ -z "${g_regression_params}" ]; then
		...
		echo "g_regression_params: ${g_regression_params}"
	fi
	...
	if [ -z "${g_reg_conf_vars}" ]; then
		...
		echo "g_reg_conf_vars: ${g_reg_conf_vars}"
	fi
...
#       mPath="${GitRepo}/MSMAll/scripts"

# matlab -nojvm -nodisplay -nosplash <<M_PROG
# addpath '$mPath'; MSMregression('${inputspatialmaps}','${inputdtseries}','${inputweights}','${outputspatialmaps}','${outputweights}','${Caret7_Command}','${Method}','${Params}','${VN}',${nTPsForSpectra},'${BC}');
# M_PROG
# echo "addpath '$mPath'; MSMregression('${inputspatialmaps}','${inputdtseries}','${inputweights}','${outputspatialmaps}','${outputweights}','${Caret7_Command}','${Method}','${Params}','${VN}',${nTPsForSpectra},'${BC}');"
