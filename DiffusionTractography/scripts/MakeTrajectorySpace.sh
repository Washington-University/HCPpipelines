...
applywarp --rel --interp=nn -i "$T1wFolder"/wmparc_1mm.nii.gz -r "$T1wFolder"/"$T1wImage"_"$DiffusionResolution" --premat=$FSLDIR/etc/flirtsch/ident.mat -o "$T1wFolder"/"$wmparc"_"$DiffusionResolution"
...
fslmaths "$T1wFolder"/"$NativeFolder"/"$Subject".L.ribbon.nii.gz -add "$T1wFolder"/"$NativeFolder"/"$Subject".R.ribbon.nii.gz "$T1wFolder"/ribbon_"$DiffusionResolution".nii.gz
...
fslmaths "$T1wFolder"/"$ribbon"_"$DiffusionResolution" -sub "$T1wFolder"/"$ribbon"_"$DiffusionResolution" "$ROIsFolder"/temp/trajectory
fslmaths "$ROIsFolder"/temp/trajectory -mul 1 "$ROIsFolder"/temp/delete_mask.nii.gz
fslmaths "$ROIsFolder"/temp/trajectory -mul 1 "$ROIsFolder"/temp/CC_mask.nii.gz
...
for Structure in $wmparcStructuresToDeleteSTRING ; do
  fslmaths "$T1wFolder"/"$wmparc"_"$DiffusionResolution" -thr $Structure -uthr $Structure -bin "$ROIsFolder"/temp/$Structure
  fslmaths "$ROIsFolder"/temp/$Structure -add "$ROIsFolder"/temp/delete_mask.nii.gz "$ROIsFolder"/temp/delete_mask.nii.gz
done
fslmaths "$ROIsFolder"/temp/delete_mask.nii.gz  -bin -sub 1 -mul -1 "$ROIsFolder"/temp/inverse_delete_mask.nii.gz
...
for Structure in $wmparcStructuresToKeepSTRING ; do
  fslmaths "$T1wFolder"/"$wmparc"_"$DiffusionResolution" -thr $Structure -uthr $Structure -bin "$ROIsFolder"/temp/$Structure
  fslmaths "$ROIsFolder"/temp/$Structure -mul $Structure -add "$ROIsFolder"/temp/trajectory "$ROIsFolder"/temp/trajectory
done

fslmaths "$ROIsFolder"/temp/trajectory -bin -sub 1 -mul -1 "$ROIsFolder"/temp/inverse_trajectory_mask
...
for Structure in $RibbonStructures ; do
  fslmaths "$T1wFolder"/"$ribbon"_"$DiffusionResolution" -thr $Structure -uthr $Structure -bin "$ROIsFolder"/temp/$Structure
  fslmaths "$ROIsFolder"/temp/$Structure -mas "$ROIsFolder"/temp/inverse_trajectory_mask -mul $Structure -add "$ROIsFolder"/temp/trajectory "$ROIsFolder"/temp/trajectory
done
...
for Structure in $CorpusCallosumToAdd ; do 
  fslmaths "$T1wFolder"/"$wmparc"_"$DiffusionResolution" -thr $Structure -uthr $Structure -bin "$ROIsFolder"/temp/$Structure
  fslmaths "$ROIsFolder"/temp/$Structure -add "$ROIsFolder"/temp/CC_mask.nii.gz "$ROIsFolder"/temp/CC_mask.nii.gz
done

fslmaths "$ROIsFolder"/temp/trajectory -bin -sub 1 -mul -1 "$ROIsFolder"/temp/inverse_trajectory_mask.nii.gz
fslmaths "$ROIsFolder"/temp/CC_mask.nii.gz -mas "$ROIsFolder"/temp/inverse_trajectory_mask.nii.gz "$ROIsFolder"/temp/CC_to_add
fslmaths "$ROIsFolder"/temp/CC_to_add -mul 2 -add "$ROIsFolder"/temp/trajectory "$ROIsFolder"/temp/trajectory
fslmaths "$ROIsFolder"/temp/trajectory -mas "$ROIsFolder"/temp/inverse_delete_mask.nii.gz "$ROIsFolder"/temp/trajectory

fslmaths "$ROIsFolder"/temp/trajectory -bin "$ROIsFolder"/Whole_Brain_"$trajectory"_ROI_"$DiffusionResolution"
fslmaths "$ROIsFolder"/Whole_Brain_"$trajectory"_ROI_"$DiffusionResolution" -sub 1 -mul -1 "$ROIsFolder"/Whole_Brain_"$trajectory"_invROI_"$DiffusionResolution"
...
fslmaths "$ROIsFolder"/temp/trajectory.nii.gz -sub "$ROIsFolder"/temp/trajectory.nii.gz -add "$T1wFolder"/L_Cerebral_"$trajectory"_"$DiffusionResolution".nii.gz -bin "$ROIsFolder"/L_Cerebral_"$trajectory"_ROI_"$DiffusionResolution"
fslmaths "$ROIsFolder"/temp/trajectory.nii.gz -sub "$ROIsFolder"/temp/trajectory.nii.gz -add "$T1wFolder"/R_Cerebral_"$trajectory"_"$DiffusionResolution".nii.gz -bin "$ROIsFolder"/R_Cerebral_"$trajectory"_ROI_"$DiffusionResolution"
fslmaths "$ROIsFolder"/L_Cerebral_"$trajectory"_ROI_"$DiffusionResolution" -sub 1 -mul -1 "$ROIsFolder"/L_Cerebral_"$trajectory"_invROI_"$DiffusionResolution"
fslmaths "$ROIsFolder"/R_Cerebral_"$trajectory"_ROI_"$DiffusionResolution" -sub 1 -mul -1 "$ROIsFolder"/R_Cerebral_"$trajectory"_invROI_"$DiffusionResolution"
