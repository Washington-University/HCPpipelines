...
if [[ -r ${T1wFolder}/${T2wImage}.nii.gz ]] ; then
...
if [[ -r ${AtlasSpaceFolder}/${AtlasSpaceT2wImage}.nii.gz ]] ; then
...
for GrayordinatesResolution in ${GrayordinatesResolutions} ; do
  ...
  if [[ -r ${AtlasSpaceFolder}/${AtlasSpaceT2wImage}.${GrayordinatesResolution}.nii.gz ]] ; then
...
for Hemisphere in L R ; do
  ...
  for Map in sulc@sulc@Sulc thickness@thickness@Thickness curv@curvature@Curvature ; do
    ...
    mathexpr="var * -1" # state expression in a seprate variable to avoid globbing of "*"
    ${CARET7DIR}/wb_command -metric-math "${mathexpr}" "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$wbname".native.shape.gii -var "var" "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$wbname".native.shape.gii
