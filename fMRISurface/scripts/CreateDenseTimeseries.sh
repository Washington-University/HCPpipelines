...
if [ -f "$NameOffMRI"_AtlasSubcortical_s"$SmoothingFWHM".nii.gz ] ; then
	gunzip "$NameOffMRI"_AtlasSubcortical_s"$SmoothingFWHM".nii.gz
fi

$Caret5_Command -create-cifti-dense-timeseries -time-step "$TR_vol" -input-volume "$NameOffMRI"_AtlasSubcortical_s"$SmoothingFWHM".nii $CIFTIAtlasString -output-cifti-file "$OutputAtlasDenseTimeseries".dtseries.nii 
# # # # echo "$Caret5_Command -create-cifti-dense-timeseries -time-step "$TR_vol" -input-volume "$NameOffMRI"_AtlasSubcortical_s"$SmoothingFWHM".nii $CIFTIAtlasString -output-cifti-file "$OutputAtlasDenseTimeseries".dtseries.nii"
gzip "$NameOffMRI"_AtlasSubcortical_s"$SmoothingFWHM".nii

gunzip "$NameOffMRI"_Subcortical_s"$SmoothingFWHM".nii.gz
$Caret5_Command -create-cifti-dense-timeseries -time-step "$TR_vol" -input-volume "$NameOffMRI"_Subcortical_s"$SmoothingFWHM".nii $CIFTIString -output-cifti-file "$OutputDenseTimeseries".dtseries.nii
# # # # echo "$Caret5_Command -create-cifti-dense-timeseries -time-step "$TR_vol" -input-volume "$NameOffMRI"_Subcortical_s"$SmoothingFWHM".nii $CIFTIString -output-cifti-file "$OutputDenseTimeseries".dtseries.nii"
