...
opts_AddMandatory '--output-string' 'OutputSTRING' 'string' "output string identifier"
...
matlab_argarray=("$StudyFolder" "$SubjListRaw" "$fMRIListRaw" "$ConcatName" "$fMRIProcSTRING" "$OutputfMRIName" "$OutputSTRING" "$OutputPrefix" "$RegString" "$LowResMesh" "$TR" "$PFMFolder")
