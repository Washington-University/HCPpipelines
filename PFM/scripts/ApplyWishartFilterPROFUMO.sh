...
case "$MatlabMode" in
    (0)
        matlab_cmd=("$this_script_dir/Compiled_ApplyWishartFilterProfumo/run_ApplyWishartFilterPROFUMO.sh" "$MATLAB_COMPILER_RUNTIME" "${matlab_argarray[@]}")        log_Msg "running compiled matlab command: ${matlab_cmd[*]}"
        "${matlab_cmd[@]}"
        ;;
    (1 | 2)
        matlab_args=""
        ...
        matlabcode="
            ...
            ApplyWishartFilterPROFUMO($matlab_args);"
